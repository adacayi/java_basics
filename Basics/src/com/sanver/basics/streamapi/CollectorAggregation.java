...
public class CollectorAggregation {
	...
	public static void main(String[] args) {
		Person[] people = { new Person("Ahmet", 3), new Person("Mustafa", 3), new Person("Fatima", 6) };
		...
		System.out.println("Average age is " + supplier.get().mapToInt(p -> p.age).average().getAsDouble());
		System.out.println("Minimum age is "
				+ supplier.get().collect(Collectors.minBy(Comparator.comparing((Person p) -> p.age))).get().age);
		// This is shorter and you don’t need to define a Comparator
		System.out.println("Minimum age is " + supplier.get().mapToInt(p -> p.age).min().getAsInt());
		System.out.println("Maximum age is "
				+ supplier.get().collect(Collectors.maxBy(Comparator.comparing((Person p) -> p.age))).get().age);
		System.out.println("Maximum age is " + supplier.get().mapToInt(p -> p.age).max().getAsInt());
